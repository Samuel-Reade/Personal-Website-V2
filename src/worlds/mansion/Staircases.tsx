...
/**
 * The face of the side wall's dado, in |x|: the wall's own inner face is at
 * 14.0, and the wainscot panels stand a little proud of it. The flight starts
 * against this wall, and its outer edge is held flush to this line rather
 * than allowed to run into it — at the full outer radius the first treads
 * ended twenty centimetres inside the masonry.
 */
const WALL_FACE_X = HALL_MAX_X - WALL_THICKNESS / 2 - 0.65;
/**
 * The step at which the outer balustrade begins. Before it the flight is
 * against the wall — a wall-mounted handrail serves there — and by it the
 * outer edge has swept half a metre clear, which is room for a newel and a
 * run of balusters. Its foot newel used to stand at the very first step, which
 * put it, and the rail it carried, inside the wall and the pilaster there.
 */
const OUTER_RAIL_FROM_STEP = 9;
/** How far the wall rail stands off the dado. */
const WALL_RAIL_GAP = 0.1;
...
/** The flight's outer radius at angle `a`: the full width, capped at the wall. */
const outerRadiusAt = (a: number): number =>
  Math.min(STAIR_OUTER_RADIUS, (WALL_FACE_X - STAIR_PIVOT_X) / Math.cos(a));
/** The radius of a line held at `x` — the wall rail's path, in polar terms. */
const radiusAlongX = (x: number) => (a: number): number => (x - STAIR_PIVOT_X) / Math.cos(a);
...
function buildFlight(side: 1 | -1) {
  ...
  /**
   * A sector box between angles a0..a1, from radius rIn out to rOut0 (at a0)
   * and rOut1 (at a1), its bottom at yB0 (at a0) rising to yB1 (at a1), its
   * top likewise yT0..yT1 — helical faces where the two differ, flat where
   * they don't. Only the faces asked for.
   */
  const wedge = (
    out: number[],
    rIn: number, rOut0: number, rOut1: number, a0: number, a1: number,
    yB0: number, yB1: number, yT0: number, yT1: number,
    faces: Faces
  ) => {
    const fiB = P(rIn, a0, yB0), foB = P(rOut0, a0, yB0), fiT = P(rIn, a0, yT0), foT = P(rOut0, a0, yT0);
    const biB = P(rIn, a1, yB1), boB = P(rOut1, a1, yB1), biT = P(rIn, a1, yT1), boT = P(rOut1, a1, yT1);
    const am = (a0 + a1) / 2;
  ...
  for (let i = 0; i < STEP_COUNT; i++) {
    ...
    // The outer edge: the full width, except along the wall, where it is
    // held flush to the dado.
    const nose = a0 - NOSE_ANGLE;
    const rOutNose = outerRadiusAt(nose);
    const rOut0 = outerRadiusAt(a0);
    const rOut1 = outerRadiusAt(a1);

    // The tread: a thin slab with its nosing overhanging the riser.
    wedge(treads, rIn, rOutNose, rOut1, nose, a1, under, under, top, top, {
    ...
    wedge(body, rIn, rOut0, rOut1, a0, a1, s0, s1, under, under, {
      bottom: s0 > 0 || s1 > 0, inner: true, outer: true, back: i === STEP_COUNT - 1,
    });
    // The riser: the body's front, from the tread below up to this one.
    quad(body, P(rIn, a0, previousTop), P(rOut0, a0, previousTop), P(rOut0, a0, under), P(rIn, a0, under), neg(tangent(a0)));

    // The runner: over the tread, and down the riser and nosing in front of it.
    wedge(runner, runIn, runOut, runOut, nose, a1, top, top, top + RUNNER_THICKNESS, top + RUNNER_THICKNESS, {
      top: true, inner: true, outer: true,
    });
    wedge(runner, runIn, runOut, runOut, nose - RUNNER_THICKNESS / R_MID, nose, previousTop + RUNNER_THICKNESS, previousTop + RUNNER_THICKNESS, top + RUNNER_THICKNESS, top + RUNNER_THICKNESS, {
...
/**
 * A handrail's path, holding RAIL_HEIGHT over the treads: a helix when the
 * radius is constant, and whatever line the radius function traces otherwise
 * — the wall rail runs at a fixed x, which in polar terms is a radius that
 * grows with the angle.
 */
class RailCurve extends THREE.Curve<THREE.Vector3> {
  constructor(
    private side: 1 | -1,
    private radius: (a: number) => number,
    private from: number,
    private to: number
  ) {
    super();
  ...
  getPoint(t: number, target = new THREE.Vector3()): THREE.Vector3 {
    ...
    return target.set(
      this.side * (STAIR_PIVOT_X + r * Math.cos(a)),
      railLine(a),
      STAIR_PIVOT_Z - r * Math.sin(a)
    );
  }
 ...
 * balustrade — a baluster on every tread, a handrail sweeping up as one helix,
 * a newel at each end — along the inner edge the whole way, and along the
 * outer edge from where the sweep carries it clear of the wall. Along the
 * wall itself the outer edge is flush to the dado and a handrail on brackets
 * runs on the wall instead, the way a stair against a wall is actually
 * railed.
 *
...
function Flight({ side }: { side: 1 | -1 }) {
  ...
  const innerRadius = STAIR_INNER_RADIUS + BALUSTER_INSET;
  const outerRadius = STAIR_OUTER_RADIUS - BALUSTER_INSET;
  /** The inner rail runs from just before the first riser to the head line. */
  const innerFrom = -1.2 * STEP_ANGLE;
  const railTo = Math.PI / 2 - 0.6 * STEP_ANGLE;
  /** The outer rail from its newel, a third of the way into its first step. */
  const outerFrom = (OUTER_RAIL_FROM_STEP + 0.3) * STEP_ANGLE;
  /** The wall rail, from just before the first riser to hand over to the newel. */
  const wallFrom = -0.8 * STEP_ANGLE;
  const wallTo = (OUTER_RAIL_FROM_STEP + 0.6) * STEP_ANGLE;
  const wallX = WALL_FACE_X - WALL_RAIL_GAP;

  const rails = useMemo(
    () => [
      new THREE.TubeGeometry(new RailCurve(side, () => innerRadius, innerFrom, railTo), 48, RAIL_RADIUS, 6, false),
      new THREE.TubeGeometry(new RailCurve(side, () => outerRadius, outerFrom, railTo), 40, RAIL_RADIUS, 6, false),
      new THREE.TubeGeometry(new RailCurve(side, radiusAlongX(wallX), wallFrom, wallTo), 16, RAIL_RADIUS * 0.8, 6, false),
    ],
    [side, innerRadius, outerRadius, innerFrom, railTo, outerFrom, wallFrom, wallTo, wallX]
  );
  ...
  const outerNewelTread = (OUTER_RAIL_FROM_STEP + 1) * RISER;
  ...
  return (
    <group>
      ...
      {/* Balusters: one per tread on the inner edge, and on the outer edge
          once the flight is clear of the wall, each standing on its tread. */}
      {Array.from({ length: STEP_COUNT }, (_, i) => {
        const a = (i + 0.5) * STEP_ANGLE;
        const top = (i + 1) * RISER;
        const radii = i > OUTER_RAIL_FROM_STEP ? [innerRadius, outerRadius] : [innerRadius];
      ...
      {/* Brackets holding the wall rail off the dado. */}
      {[0.15, 0.5, 0.85].map((t) => {
        const a = wallFrom + (wallTo - wallFrom) * t;
        const r = radiusAlongX(wallX)(a);
        return (
          <mesh
            key={t}
            material={railMaterial}
            position={at(r + WALL_RAIL_GAP / 2, a, railLine(a) - 0.04)}
            rotation={[0, side * a, Math.PI / 2]}
          >
            <cylinderGeometry args={[0.025, 0.025, WALL_RAIL_GAP + 0.06, 5]} />
          </mesh>
        );
      })}

      {/* Newels: at the foot of the inner rail on the floor before the first
          riser; where the outer rail begins, on its tread; and one at each
          head, on the top tread — those sized so their tops and caps sit where
          the gallery's newels put theirs, so the flight's rail and the
          gallery's die into the same kind of post. */}
      <Newel
        position={at(innerRadius, innerFrom, 0)}
        height={railLine(innerFrom) + 0.16}
        postMaterial={railMaterial}
        capMaterial={balusterMaterial}
      />
      <Newel
        position={at(outerRadius, outerFrom, outerNewelTread)}
        height={railLine(outerFrom) - outerNewelTread + 0.16}
        postMaterial={railMaterial}
        capMaterial={balusterMaterial}
      />
      {[innerRadius, outerRadius].map((radius) => (
        <Newel
          key={`head${radius}`}
...
function Gallery() {
  ...
  /**
   * The short run across the shelf's front dies into the flight's own inner
   * head newel, which stands on the top tread just past x = pivot + 0.13 —
   * the stub reaches to its centre so the two read as one piece of joinery.
   */
  const stubTo = STAIR_PIVOT_X + 0.15;
  /**
